...
export function CentersSection({ cards }: CentersSectionProps): JSX.Element {
  return (
    <section className="border-t border-(--ca-border) bg-(--ca-surface-soft) py-12">
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-4">
          {cards.map((card) => (
            ...
            >
              ...
              <div className="p-4">
                <h3 className="text-3xl font-black tracking-[-0.03em] text-(--ca-text)">
                  {card.title}
                </h3>
                <p className="mt-2 text-sm leading-6 text-(--ca-text-muted)">
                  {card.description}
                </p>
                <a
                  href="#"
                  className="mt-3 inline-flex text-sm font-semibold text-(--ca-brand-900)"
                >
                  {card.cta ?? "Learn More"} →
                </a>
              </div>
