...
export function TestimonialsAppointmentSection({
  testimonials,
}: TestimonialsAppointmentProps): JSX.Element {
  return (
    <section className="bg-(--ca-surface) py-12">
      <div className="mx-auto grid w-full max-w-7xl gap-6 px-4 sm:px-6 lg:grid-cols-[1.18fr_0.82fr] lg:px-8">
        <article>
          <h2 className="text-5xl font-black tracking-[-0.04em] text-(--ca-text)">
            Patient Experiences
          </h2>
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {testimonials.map((item) => (
              <div
                key={item.name}
                className="rounded-2xl border border-(--ca-border) bg-(--ca-surface) p-5"
              >
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <div className="text-lg font-black text-(--ca-text)">
                      {item.name}
                    </div>
                    <div className="text-xs text-(--ca-text-subtle)">
                      {item.role}
                    </div>
                  </div>
                </div>
                <p className="mt-4 text-sm leading-7 text-(--ca-text-muted)">
                  “{item.quote}”
                </p>
              </div>
            ))}
        ...
        <aside className="rounded-2xl border border-(--ca-border) bg-(--ca-surface) p-6 shadow-[0_20px_50px_rgba(17,33,58,0.08)]">
          <h3 className="text-5xl font-black tracking-[-0.04em] text-(--ca-text)">
            Book Appointment
          </h3>
