  ...
  return (
    <section className="bg-[var(--ca-surface)] py-12">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <h2 className="text-center text-5xl font-black tracking-[-0.04em] text-[var(--ca-text)]">Meet Our Specialists</h2>
        <div className="mt-8 grid gap-4 lg:grid-cols-4">
          {cards.map((card) => (
            <article key={card.title} className="overflow-hidden rounded-2xl border border-[var(--ca-border)] bg-[var(--ca-surface)]">
              ...
              <div className="p-4">
                <h3 className="text-2xl font-black tracking-[-0.03em] text-[var(--ca-text)]">{card.title}</h3>
                <p className="mt-1 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--ca-text-subtle)]">{card.role}</p>
