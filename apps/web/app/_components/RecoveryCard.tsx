...
export default function RecoveryCard({ identifier, setIdentifier, onSubmit, isLoading }: Props) {
  return (
    <div className="bg-white rounded-xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] max-w-[540px] w-full p-14 border border-white/50">
      <h1 className="text-[34px] font-black text-[#1E293B] mb-4 tracking-tight">Account Recovery</h1>
      <p className="text-[#64748B] text-[15px] mb-10 leading-relaxed font-medium">
        To maintain a HIPPA compliant environment, enter your credentials below to begin the identity verification
        process.
      </p>
      ...
      <form onSubmit={onSubmit} className="space-y-8">
        <div>
          ...
          <p className="text-[10px] text-[#94A3B8] mt-3 font-bold leading-tight">
            Recovery instructions will be sent via your primary communication channel.
          </p>
        </div>
      ...
      <div className="mt-10 bg-[#F0F7FF] rounded-xl p-6 flex gap-4 items-start border border-[#D0E4FF]">
        ...
        <p className="text-[11.5px] text-[#475569] leading-[1.6] font-medium">
          <span className="font-black text-[#1E293B] uppercase tracking-tighter mr-1">Security Protocol:</span>
          If a matching account exists in our health systems, a verification code has been dispatched. Please check your
